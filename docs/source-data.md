...
Pass `generateArmorVariants: true` to a snapshot search to derive ordinary Qurious Crafting variants automatically for the requested armor records. The default generation limit is the confirmed seven operations per armor. `maxOperations` and `maxVariants` are explicit preview/test limits; they are not game rules and are not used by default. Skill+ and Slot+ special modes are outside the current planner scope.
